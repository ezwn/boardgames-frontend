  ...
  moves.forEach(move => {
    if (move.from && move.to && !move.canceled) {
      ...
      if (!isEmpty(move.to.piece)) {
        if (isWhite(move.to.piece)) state.jail.white += move.to.piece;
        else state.jail.black += move.to.piece;
      }
      state.board = squarePut(
        state.board,
        move.to.c,
        move.to.l,
        move.from.piece
...
export const isValidMove = (state, move) => {
  ...
  if (isEmpty(from.piece)) return false;

  if (isWhite(from.piece) && isWhite(to.piece)) return false;

  if (isBlack(from.piece) && isBlack(to.piece)) return false;
